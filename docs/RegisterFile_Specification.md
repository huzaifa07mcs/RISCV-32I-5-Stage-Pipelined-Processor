...
Read returns the **old** value, since reads are combinational off current `regfile` contents and the write only updates `regfile` at the next clock edge. This is standard single-cycle behavior.
> Note: this becomes a data hazard in the pipelined version and will be resolved later by the Forwarding Unit — flagged here for future reference.
...
- Read x1 and x2 as read_addr1 and read_addr2 where rs1 == rs2 (same address on both ports) — confirm both outputs match.
- Write and read the same register in the same clock cycle (write_addr == read_addr1) — confirm read_data returns the **old** value, not write_data.
- RegWrite == 0 with valid write_addr and write_data — confirm no write occurs.
